...
export const cicd = createModule({
  ...
  level: 'intermediate',
  icon: '🔄',
  description: 'Continuous integration, deployment strategies, GitHub Actions, and production release pipelines.',
  prerequisites: ['git', 'docker'],
  estimatedHours: 15,
  learningObjectives: [
    'Design CI/CD pipelines with testing gates',
  ...
  sections: [
    {
      ...
      content: `### CI pipeline stages
1. **Lint** — code style, type checking
2. **Unit tests** — fast, isolated
3. **Integration tests** — DB, external services
4. **Build** — Docker image, artifact
...
Replace instances gradually. Default K8s strategy. Brief mixed-version period.

### Blue-green
Two identical environments. Switch traffic instantly. Fast rollback. 2x resource cost.

### Canary
Route small % traffic to new version. Monitor metrics. Gradually increase. Safest.

### Feature flags
Deploy code without enabling feature. Decouple deploy from release.
